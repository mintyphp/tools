...
class DebuggerTool
{
    ...
    {
        (new self())->execute(
            \MintyPHP\Debugger::getInstance()
        );
    }

    /**
     * Execute the debugger tool
     * @param Debugger $debugger The debugger instance
     */
    public function execute(Debugger $debugger): void
    {
        $requests = $debugger->getHistory();
        echo $this->getHtml($requests);
    }
