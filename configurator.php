...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
	...
	echo "</pre>";
	if ($success) echo '<input type="button" value="OK" onClick="window.location.href=window.location.href">';
	else echo '<input type="button" value="Back" onClick="history.go(-1)">';
	die();
}
...
class Configurator
{
	...
	{
		if (!file_exists($filename)) $filename .= '.template';
